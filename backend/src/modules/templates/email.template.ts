...
export function baseTemplate(title: string, content: string, accentColor: string): string {
  return `
<!DOCTYPE html>
<html>
<head>
...
export function invoiceDetails(data: EmailData): string {
...
export function dueDateTemplate(data: EmailData): string {
...
export function threeDayTemplate(data: EmailData): string {
  const safeName = escapeHtml(data.clientName)
  return baseTemplate(
    'Payment Follow-Up',
    `<p style="color:#cbd5e1;line-height:1.6;font-size:15px;">Hi ${safeName},</p>
...
export function sevenDayTemplate(data: EmailData): string {
  const safeName = escapeHtml(data.clientName)
  return baseTemplate(
...
export function fourteenDayTemplate(data: EmailData): string {
  const safeName = escapeHtml(data.clientName)
  return baseTemplate(
...
export const subjects: Record<number, (invoiceId: string) => string> = {
...
export function getVerificationEmailContent(verifyUrl: string): { subject: string; html: string } {
  const content = `
...
export function getPasswordResetEmailContent(resetUrl: string): { subject: string; html: string } {
  const content = `
